...
void BackgrounfGaussianMix(Mat& src, Mat& dst)
{
...
Mat getForegroundMask(Mat image, Mat background, double th)
{
	Mat fgMask, kernel, fgMask_th, BlurImg, BurBg, Ycbcr_Img, Ycbcr_Bg,diffImg, diffImg2, diffImg3, fgMask_fill;
	Mat img_channel[3],bg_channel[3];
	//double dist;
	float alpha = 0.01;
	int iterations = 3;
	...
	absdiff(img_channel[1], bg_channel[1], diffImg);
	absdiff(img_channel[2], bg_channel[2], diffImg2);
	absdiff(img_channel[0], bg_channel[0], diffImg3);
	//diffImg = diffImg * 0.5 + diffImg2 * 0.5;
	diffImg = diffImg * 0.5 + diffImg2 * 0.5 + diffImg3 * 0.2;
	fgMask_th = Mat::zeros(diffImg.size(), CV_8UC1);
	//addWeighted(BurBg, alpha, diffImg, 1 - alpha, 0, bgframe);
	/*split(diffImg, img_channel);
	diffImg = img_channel[1] + img_channel[2];*/
	...
	////erode(fgMask_th, fgMask_th, kernel, cvPoint(-1, -1), 3);
	/*morphologyEx(fgMask_th, fgMask_th, MORPH_CLOSE, kernel, cvPoint(-1, -1), 3);
	morphologyEx(fgMask_th, fgMask, MORPH_OPEN, kernel, cvPoint(-1, -1), 3);*/
...
void ProcessImages(Mat background, Mat image, int& framecount)
{
	...
	image.copyTo(fgImg);	
	fgMask = getForegroundMask(fgImg, background, 20);
	fgImg.create(image.size(), image.type());
	fgImg = Scalar::all(0);
 	image.copyTo(fgImg, fgMask);
	//image.copyTo(facedetection);
...
int main()
{
	//string streamaddress = "rtsp://root:pass@192.168.0.51/onvif-media/media.amp";
	string streamaddress = "http://root:pass@192.168.0.51/axis-cgi/mjpg/video.cgi";
	//string streamaddress = "http://root:pass@192.168.0.51/mjpg/video.mjpg";
